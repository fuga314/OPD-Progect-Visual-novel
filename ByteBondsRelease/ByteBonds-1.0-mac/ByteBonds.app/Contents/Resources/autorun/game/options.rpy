...
define config.name = _("Byte bonds")
...
define gui.about = _p("""
Главный герой новеллы - Дэн, выпускник провинциального ВУЗа.
Получил профессию инженера-программиста.
На данный момент он работает в маленькой IT-компании по разработке медицинского оборудования.
Но всё меняется в один из самых обычных вечеров, когда он получает загадочное электронное письмо, которое может изменить его будущее.

Решится ли он на кардинальные перемены в своей жизни?
""")
...
define config.main_menu_music = "bg_main_theme.mp3"
...
define config.save_directory = "ByteBonds-1699771807"
...
init python:
    ...
    renpy.music.register_channel("second_sound", loop=False)
    renpy.music.register_channel("ai_phrase", loop=False)
